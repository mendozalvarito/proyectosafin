$(function(){
    initialize();
    //evento que se ejecuta cuando se va a buscar una ubicacion
    function eventBuscarUbicacion(e){
        //servicio que va a obtener el resultado de la busqueda
        var endpoint = "/app1/coords/buscar/";

        //valor enviado al servicio, enviamos el texto a buscar
        var params = { query:  $('#query').val()};

        //enviamos la peticion
        var searchReq = $.get(endpoint,params);

        //resultado de haber enviado la peticion
     ...
     $("#search").click(eventBuscarUbicacion);
    ...
    function displayError(error) {
        ...
        switch (error.code)
        {
            case error.PERMISSION_DENIED:errorstr = "Permission deined";
                break;
            case error.POSITION_UNAVAILABLE:errorstr = "Permission unavailable";
                break;
            case error.TIMEOUT:errorstr = "Timeout";
                break;
            case error.UNKNOWN_ERROR:error = "Unknown error";
                break;
        }

        console.log("GPS error: " + error + " - Message: " + errorstr);

        createMap(-11.022923,-68.759169);
